...
(function (module) {
  ...
  Article.numWordsByAuthor = () => {
    return Article.allAuthors().map(author => {
      let totalByAuthor = Article.all.filter(articleObject => articleObject.author === author).map(articleObject => articleObject.body.length).reduce((a, b) => a + b, 0)

      +

      Article.all.filter(articleObject => articleObject.author === author).map(articleObject => articleObject.title.length).reduce((a, b) => a + b, 0);
